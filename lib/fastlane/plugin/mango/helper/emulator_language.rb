      ...
      def self.set(lang)
        ...
        if current_locale.eql? lang
          ...
          if api < 28
            ...
            Aruba::ADB.execute('install ../settings_apk/settings_apk-debug.apk')
            Aruba::ADB.execute('shell pm grant io.appium.settings android.permission.CHANGE_CONFIGURATION')
            Aruba::ADB.execute("shell am broadcast -a io.appium.settings.locale -n io.appium.settings/.receivers.LocaleSettingReceiver --es lang #{language} --es country #{country}")
          end
        end
      end
    end
